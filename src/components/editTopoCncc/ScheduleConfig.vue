  ...
  <div class="work-schedule">
    <div class="on-off-schedule">
      ...
      <span class="on-off-group">
         <input type="button" value="开启" @click="switchChange('on')" :class="{ 'btn-selected': isOn} "/>
      <input type="button"  value="关闭" @click="switchChange('off')" :class="{ 'btn-selected': !isOn}"/>
      </span>
    </div>
    <div class="edit-schedule" :class="{'edit-status': editId !== '','off-style': !isOn}" >
      <div class="left-work">
        <span class="alert-name-tip"></span>
        <span class="work-name">工作时段名称</span>
        <input ref="workName" type="text" autocomplete="off" placeholder="周末" maxlength="20" v-model="editName" @blur="checkName">
      </div>
    ...
    <div class="show-schedule" :class="{'off-style': !isOn}">
      <div :class="{'per-schedule': true, 'edit-status': isAdd}" v-if="isAdd">
        <div class="per-schedule-content">
          <time-axis :timeList="newTime" @timeChange="timeChange"></time-axis>
        </div>
      </div>
      <div :class="{'per-schedule': true, 'edit-status': editId === item.id}" v-for="(item,index) in newSchedule.schedule" :key="item.id">
        <div class="per-schedule-tit">
          ...
          <input class="schedule-edit-btn" type="button" value="编辑" @click="editSchedule(item)"/>
          <input type="button"  value="删除" @click="removeSchedule(item.id)"/>
        </div>
        <div class="per-schedule-content">
          <div class="per-schedule-type">工作时段</div>
          <div class="per-schedule-axis">
            <time-axis :timeList="item.time_range" :isClear="isClear" @timeChange="timeChange"></time-axis>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkName() {
        const res = { bool: !this.newCollector.disp_name.match(/[&; ]/g), tip: '不能包含"&",";"和空格!' };
