  ...
  <div>
    ...
    <div class="coll-div coll-mid">
      <div class="input-div">
        ...
        <input :class="{'error-input': newCollector.disp_name === ''}" v-model="newCollector.disp_name" @input="checkName($event,'collector')" maxlength="20" autocomplete="off" type="text">
      </div>
      <div class="input-div">
        ...
        <select :class="{'error-input': nicError}" data-type="nic" @change="selectChange" v-model="newCollector.nic_addr"  class="port-input">
          <option v-for="(item,index) in servers.nices" :value="item" :key="index">{{ item }}</option>
          <option
            v-if="nicError"
      ...
      <div class="input-div">
        ...
        <select :class="{'error-input': serverError}" data-type="server" @change="selectChange" v-model="newCollector.server_id" class="port-input">
          <option :value="servers.server_id">{{ servers.server_id }}</option>
          <option
            v-if="serverError"
    ...
    <div class="coll-div coll-bottom">
      ...
      <div class="proto-list">
        ...
        <span class="proto-text ellipsis">{{ csvFiles }}</span>
      </div>
      <div class="proto-list">
      ...
      <div class="proto-list">
        ...
        <span class="proto-text ellipsis">{{ protocolInfo.config.transaction_id }}</span>
      </div>
      <div class="proto-list">
        <span class="proto-disp ellipsis">交易类型</span>
        <span class="proto-text ellipsis">{{ protocolInfo.config.trans_type }}</span>
      </div>
      <div class="proto-list">
        <span class="proto-disp ellipsis">交易渠道</span>
        <span class="proto-text ellipsis">{{ protocolInfo.config.sub_trans_type }}</span>
      </div>
      <div class="proto-list">
        <span class="proto-disp ellipsis">返回码</span>
        <span class="proto-text ellipsis">{{ protocolInfo.config.ret_code }}</span>
      </div>
      <div class="proto-list">
...
<script>
  ...
  export default {
    ...
    watch: {
      trade() {
        ...
        if (this.newCollector.nic_addr === '') {
          this.newCollector.nic_addr = this.servers.nices[0];
        }
        if (this.newCollector.target_prot === '') {
          this.newCollector.target_prot = Object.keys(this.protocols)[0];
        }
        if (this.newCollector.server_id === '') {
          this.newCollector.server_id = this.servers.server_id;
        }

        this.nicError = this.servers.nices.indexOf(this.newCollector.nic_addr) === -1;
        this.protocolError = !has(this.protocols, this.newCollector.target_prot);
        this.serverError = this.newCollector.server_id !== this.servers.server_id;
    ...
    computed: {
      protocolInfo() {
        return has(this.protocols, this.newCollector.target_prot) ? this.protocols[this.newCollector.target_prot]
          : {
            csv_files: [],
            config: {
              display_name: '',
      ...
      csvFiles() {
        let csv = '';
        this.protocolInfo.csv_files.forEach((d) => { csv += d.name; });
        return csv;
      },
    },
    methods: {
      ...
      selectChange(e) {
        if (e.target.className.indexOf('error-input') > -1) {
          switch (e.target.getAttribute('data-type')) {
            case 'server': this.serverError = false;
              break;
            case 'nic': this.nicError = false;
              break;
            case 'protocol': this.protocolError = false;
              break;
            default:return;
          }
        }
