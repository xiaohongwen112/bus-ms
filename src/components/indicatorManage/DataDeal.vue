...
export default {
  ...
  created() {
    ...
    let finalData = Array.isArray(this.dataT) ? _.cloneDeep(this.dataT[this.dataIndex]) : _.cloneDeep(this.dataT);
    if (this.setType === 'indicator') finalData = Array.isArray(this.dataT) ? _.cloneDeep(this.dataT[this.dataIndex]) : _.cloneDeep(this.dataT);
    if (finalData.if && !Array.isArray(finalData.if)) {
      this.normalOne.if = [];
      this.normalOne.if[0] = Object.assign({}, this.normalOne.if[0], finalData.if);
      this.normalOne.else = Object.assign({}, this.normalOne.else, finalData.else);
      this.currentCondition = 2;
      // 判断里面的field是不是对象
      if (finalData.if.field && !Array.isArray(finalData.if.field)) {
        this.normalOne.if[0].field = [];
        this.normalOne.if[0].field[0] = finalData[this.dataIndex].if.field;
      }
      if (finalData.else.field && !Array.isArray(finalData.else.field)) {
        if (finalData.else.field.length === 0) {
          return;
        }
        this.normalOne.else.field = [];
        this.normalOne.else.field[0] = finalData.else.field;
      }
    } else if (finalData.if && Array.isArray(finalData.if)) {
      if (finalData.if.length === 0) return;
      this.normalOne.if = [].concat(finalData.if);
      this.normalOne.else = Object.assign({}, this.normalOne.else, finalData.else);
      this.currentCondition = 3;
      for (let i = 0; i < finalData.if.length; i += 1) {
        if (finalData.if[i].field && !Array.isArray(finalData.if[i].field)) {
          this.normalOne.if[i].field = [];
          this.normalOne.if[i].field[0] = finalData.if[i].field;
        }
      }
      if (finalData.else.field && !Array.isArray(finalData.else.field)) {
        this.normalOne.else.field = [];
    ...
    if (finalData.field && !Array.isArray(finalData.field)) {
      this.normalOne.field = [];
      this.normalOne.field[0] = Object.assign({}, finalData.field);
      this.currentCondition = 0;
    } else if (finalData.field && Array.isArray(finalData.field)) {
      if (finalData.field.length === 0) {
        return;
      }
      this.normalOne.field = [].concat(finalData.field);
      this.currentCondition = 0;
    }
  },
  methods: {
    ...
    close() {
      // this.$store.dispatch('setData', this.$store.state.temporaryData.normalizes);
      // this.$emit('showValueTherr');
      this.$emit('close');
    },
    clearOne(index) {
    ...
    conditionFieldAdd(num) {
      if (num === 0) {
        this.normalOne.field.push({ '@op': 'MAX', '@item1': '' });
      }
      if (num === 1) {
        this.normalOne.if[0].field.push({ '@op': 'MAX', '@item1': '' });
      }
      if (num === 2) {
        this.normalOne.else.field.push({ '@op': 'MAX', '@item1': '' });
      }
      if (num >= 3) {
        this.normalOne.if[num - 2].field.push({ '@op': 'MAX', '@item1': '' });
      }
    },
    ...
    addCondition() {
      ...
      if (this.currentCondition === 2) {
        this.normalOne.else.field = [{ '@op': 'MAX', '@item1': '' }];
      }
      if (this.currentCondition > 2) {
        // this.normalOne.if.push({ '@op': 'notNull' });
        this.normalOne.if.splice(this.currentCondition - 1);
        this.normalOne.if.splice(this.currentCondition - 2, 1, { '@op': 'notNull', '@item1': 'Stsld', field: [{ '@op': 'MAX', '@item1': '' }] });
