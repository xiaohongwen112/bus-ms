<template>
  <div class="cm-select" :class="[type, {'disable': disable}]" :style="`width: ${width}px`" v-clickoutside="closeUL">
    <div class="input-container" @click="choose">
      <input type="text" :value="value" :disabled="disable" readOnly="readOnly" :placeholder="placeholder"/>
      <i class="icon-bms-tridown"></i>
    </div>
    <CmOption v-show="ulShow" :list="list" :selectOption="value" :height="height" 
    @receive="changeVal" :disabledLi="disabledLi" :position="position" :scaleWidth="scaleWidth"></CmOption>
  </div>
</template>
...
export default{
  ...
  props: {
    ...
    selectVal: { type: [String, Number], default: '' }, // 选中值
  ...
  methods: {
    changeVal(val) {
      this.value = typeof val === 'object' ? val.name : val;
      this.ulShow = false;
