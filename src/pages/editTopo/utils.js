...
import Group from '@/components/editTopo/models/Group';
 ...
 * @param {*} data
 */
export const formatBaseline = data => ({
  health: nullofBaseline(data.health, '%'),
  duration: nullofBaseline(data.duration, 'ms'),
  rr_rate: nullofBaseline(data.rr_rate, '%'),
  succ_rate: nullofBaseline(data.succ_rate, '%'),
  trans_count: [nullofBaseline(data.max_transcount, '笔'), nullofBaseline(data.min_transcount, '笔')],
});
...
export const alertTypeList = [
  { name: '所有', val: 'all' },
  { name: '健康度', val: 'health' },
  { name: '响应时间', val: 'duration' },
  { name: '响应率', val: 'rr_rate' },
  { name: '交易量', val: 'trans_count' },
  { name: '成功率', val: 'succ_rate' },
  { name: '返回码', val: 'return_code' },
];
 ...
 * return 00:00 - 24:00
 */
export const mins2str = (val) => {
  const ms = val * 60 * 1000;
  const hour = val === 24 * 60 ? '24' : String(moment.duration(ms).hours()).padStart(2, '0');
  const min = String(moment.duration(ms).minutes()).padStart(2, '0');
  return `${hour}:${min}`;
};

/**
 * HH:mm 格式转分钟
 * @param {*} str 00:00 - 24:00
 * return 0 - 1440
 */
export const str2mins = (str) => {
  const hour = Number(str.split(':')[0]);
  const min = Number(str.split(':')[1]);
  return 60 * hour + min;
};
 ...
 * @param {*} area 区
 */
export const jointAddr = (province, city, area) => {
  const tmpAddr = [province];
  if (city !== '' && city !== '市') {
    tmpAddr.push(city);
    if (area !== '' && area !== '区') {
      tmpAddr.push(area);
