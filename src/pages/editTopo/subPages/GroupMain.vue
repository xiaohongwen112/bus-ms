  ...
  <div>
    ...
    <breadcrumb :title="'可视化服务图设计'" :crumbList="crumbList" :appName="$store.state.appName">
      <div slot="btnGroup" :class="[isButtonDisabled ? '' : 'disableAppBtn', 'topo-btn-group']">
        ...
        <input :class="{'export-disable':!VisitTopovExport}" type="button" value="导出拓扑图" ref="export"/>
      </div>
    </breadcrumb>
    ...
    <SourceConfig v-if="showSourceConfig" :edge="currentEdge" />
    <TargetConfig v-show="showTargetConfig" :node="selectedNode" />
...
import Node from '@/components/editTopo/svg/GroupNode';
import Group from '@/components/editTopo/svg/Group';
import SourceConfig from '@/components/editTopo/SourceConfig';
import TargetConfig from '@/components/editTopo/TargetConfig';
import DialogModal from '@/components/editTopo/DialogModal';
import ScrollBar from '@/components/common/ScrollBar';
import Loading from '@/components/common/Loading';

import NodeClass from '@/components/editTopo/models/GroupNode';
import GroupClass from '@/components/editTopo/models/Group';
...
import { CmTip } from '@/components/basic/index';
...
export default {
  ...
  computed: {
    ...
    showSourceConfig() {
      return this.$store.state.showSourceConfig;
    },
    showTargetConfig() {
      return this.$store.state.showTargetConfig;
    },
    currentEdge() {
  ...
  methods: {
    ...
    */
    ...
    saveSourceConfig(data, name) {
      console.log('saveSourceConfig', data, name);
        ...
        event.addEventListener('ack', () => {
          _this.$store.dispatch('applyDataPath', {
            ...
            callback: (data) => {
              ...
              _this.finish = data.msg || `${time} 应用已经完成！可关闭窗口`;
              _this.showApplyOk = true;
              this.refreshAfterApply = data.code === 0;
              if (data.msg) {
                event.close();
              }
            },
    ...
    onApplyOk() {
      ...
      // this.$store.dispatch('saveWorkTime', { type: 'apply' });
  ...
  mounted() {
    this.VisitTopovExport = this.session$.newpermissions.topov_export;
...
<style scoped>
.topo-btn-group input[type="button"].export-disable{
  outline: none;
  cursor: default;
